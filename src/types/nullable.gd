...
func get_value() -> Variant:
	match typeof(_value_or_null):
		...
		_:
			push_error("Illegal state; value of Node[%s] has value %s of type %s" % [_permitted_type, _value_or_null, typeof(_value_or_null)])
			return null
			
func set_value(value: Variant) -> void:
	if not _value_or_null.permits_assignment_of_type(typeof(value)):
		push_error("Illegal assignment of type %s to Nullable[%s]" % _permitted_type)
		return
